...
import { resolveExportFormat } from '../../src/lib/constants.ts';
...
// regardless of the selected encoding). HEX display → hex text; any text
// display mode (ASCII/UTF-8/ANSI) → decoded text.
...
test('structured/binary choices pass through regardless of display mode', () => {
  for (const displayMode of ['HEX', 'ASCII', 'UTF8', 'ANSI'] as const) {
