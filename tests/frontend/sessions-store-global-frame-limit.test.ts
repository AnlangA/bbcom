...
afterEach(() => {
  vi.doUnmock('../../src/lib/session-store-helpers.ts');
  vi.resetModules();
...
test('store enforces the aggregate capture-byte budget and republishes an evicted resident session', async () => {
  vi.resetModules();
  vi.doMock('../../src/lib/session-store-helpers.ts', async () => {
    const actual = await vi.importActual<typeof import('../../src/lib/session-store-helpers.ts')>(
  ...
  const [{ useSessionCoreStore }] = await Promise.all([import('../../src/features/sessions/store/session-core.ts')]);
