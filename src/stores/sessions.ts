...
import {
  ...
  enterWorkspaceSessionPersistenceMode as selectWorkspaceSessionPersistence,
  type SessionPersistenceDirtyKind,
} from '../features/sessions/persistence/session-persistence-controller';
...
/**
 * Compatibility facade for session-owned state.
 *
 * Catalog decisions, document/settings mutations, capture accounting and
 * persistence orchestration live in framework-light feature modules. This
 * store intentionally retains the Vue adapters and the existing public API so
 * consumers can migrate independently without a big-bang rewrite.
 */
export const useSessionStore = defineStore('sessions', () => {
  ...
  const dirtyConfigurationGenerations = new Map<string, number>();
  ...
  let preflightRuntimeCapture: WorkspaceSessionMutationPermissions['preflightRuntimeCapture'];
  let preflightSessionRegistration: WorkspaceSessionMutationPermissions['preflightSessionRegistration'];

  let persistenceController: ReturnType<typeof createSessionPersistenceController> | null = null;
  ...
  function setWorkspaceMutationPermissions(permissions: WorkspaceSessionMutationPermissions): void {
    workspaceUserMutationsAllowed.value = permissions.userMutations === true;
    workspaceRuntimeCaptureAllowed.value = permissions.runtimeCapture === true;
    preflightRuntimeCapture = permissions.preflightRuntimeCapture;
    preflightSessionRegistration = permissions.preflightSessionRegistration;
  }
  ...
  function canAddRuntimeFrame(
    ...
    return canCaptureRuntimeEvents() && (preflightRuntimeCapture?.(sessionId, frame) ?? true);
  }
  ...
  function schedulePersist(
    kind: SessionPersistenceDirtyKind = 'config',
    dirtySessionId?: string,
  ): number {
    const generation = persistenceController?.schedulePersist(kind) ?? 0;
    if (dirtySessionId && generation > 0) {
      dirtyConfigurationGenerations.set(dirtySessionId, generation);
    }
    return generation;
  }
  ...
  const capture = createSessionCaptureController({
    ...
    scheduleFramesPersist: () => schedulePersist('frames'),
    now: nowMillis,
  ...
  function replaceSessionsFromFile(saved: PersistedSessionsFile): void {
    ...
    capture.replaceSessions(restored);
    activeSessionId.value = catalog.replace(restored, saved.activeSessionId, saved.mruSessionIds);
    workspaceRebindBySessionId.value = {};
    waveform.replaceWithEmptySessions(restored.map((session) => session.id));
    dirtyConfigurationGenerations.clear();
  }
  ...
  function mergeSessionsFromFile(saved: PersistedSessionsFile): void {
    ...
    if (restored.length > 0) {
      ...
      for (const session of restored) capture.initializeSession(session);
      waveform.addEmptySessions(restored.map((session) => session.id));
    }
  ...
  persistenceController = createSessionPersistenceController({
    serialize: (includeFrames) =>
      serializeSessionSnapshots(sessions.value, activeSessionId.value, {
        mruSessionIds: catalog.snapshotMruSessionIds(),
        includeFrames,
      }),
    replaceFromFile: replaceSessionsFromFile,
    mergeFromFile: mergeSessionsFromFile,
    setReadOnly: (readOnly) => {
      persistenceReadOnly.value = readOnly;
    },
    onPersisted: ({ dirtyGeneration }) => {
      for (const [sessionId, generation] of dirtyConfigurationGenerations) {
        if (generation <= dirtyGeneration) dirtyConfigurationGenerations.delete(sessionId);
      }
    },
  });
  ...
  const documentMutations = createSessionDocumentMutations({
    ...
    schedulePersist: (sessionId) => {
      schedulePersist('config', sessionId);
    },
  ...
  function createSession(portName: string, portConfig: SerialSession['portConfig']): string | null {
    ...
    if (!(preflightSessionRegistration?.(id, 0, 0) ?? true)) return null;
  ...
  async function removeSession(id: string): Promise<DeletedSessionSnapshot | null> {
    ...
    catalog.remove(id);
    dirtyConfigurationGenerations.delete(id);
  ...
  function isSessionConfigurationDirty(sessionId: string): boolean {
    return dirtyConfigurationGenerations.has(sessionId);
  }
  ...
  function undoLastRemovedSession(): UndoDeletedSessionResult {
    ...
    if (!(preflightSessionRegistration?.(snapshot.session.id, frameCount, captureBytes) ?? true)) {
    ...
    schedulePersist('mru', restored.id);
    lastDeletedSession.value = null;
  ...
  const persistenceReadyPromise = persistenceController.start();
  ...
  if (typeof window !== 'undefined') {
    const requestFinalFlush = () => {
      void persistenceController?.flushFinalPersistence();
    };
  ...
  return {
    ...
    ...settingsMutations,
    reorderSessions,
    flushPersistedSessions: persistenceController.flushPersistedSessions,
    flushFinalPersistence: persistenceController.flushFinalPersistence,
    whenPersistenceReady,
  };
});

/**
 * Select the workspace writer before creating the Pinia facade. This also
...
export function enterWorkspaceSessionPersistenceMode(): void {
  ...
  selectWorkspaceSessionPersistence();
}

function cloneStoppedSession(session: SerialSession): SerialSession {
  const cloned = structuredClone(toRaw(session)) as SerialSession;
