      ...
      <div v-show="!collapsed.checksum">
        <div class="checksum-grid">
          ...
          <n-select
            ...
            size="small"
          />
          <n-button size="small" block @click="calcChecksum" :disabled="!checksumInput || !isValidHexInput">
            计算
          </n-button>
          <div v-if="checksumResult" class="checksum-result">
...
async function calcChecksum() {
  ...
  for (let i = 0; i < cleaned.length; i += 2) {
    data.push(parseInt(cleaned.substring(i, i + 2), 16));
  }
  try {
