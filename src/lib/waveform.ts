...
export {
  ...
  type WaveformSampleIndexWindow,
  type WaveformTimeRange,
...
export function visibleChannelRangeInWindow(
  ...
  let min = Infinity;
  let max = -Infinity;
  const total = buf.samples.length;
  const start = Math.max(0, Math.min(total, Math.floor(startIndex)));
  const end = Math.max(start, Math.min(total, Math.floor(endIndex)));
  for (let c = 0; c < channelCount; c += 1) {
