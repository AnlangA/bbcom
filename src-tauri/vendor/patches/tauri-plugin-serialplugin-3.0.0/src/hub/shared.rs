...
/// Route a watch chunk without changing its byte stream.
///
/// The line router is observational here: it may publish a parallel URC event,
/// but `SerialEvent::Data` must retain every byte read from the serial port.
/// In particular, newlines, empty lines, and non-UTF-8 data are significant to
/// generic serial consumers.
#[doc(hidden)]
pub fn route_watch_chunk(path: &str, chunk: &[u8], state: &mut HubRoutingState) {
    ...
    for action in state.line_router.route_streaming(chunk, &[]) {
        if let RxRouteAction::UrcLine(line) = action {
            state.pending_events.push(SerialEvent::Urc {
                path: path.to_string(),
                line,
            });
